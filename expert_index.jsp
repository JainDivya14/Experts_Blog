<%-- 
    Document   : expert_index
    Created on : May 11, 2020, 8:04:31 PM
    Author     : Lenovo
--%>
...
<BODY style="background:#bdffc4;">
    ...
    <h4 class="list_header" style="background:lightgray;text-align: left;font-size:25px;">Questions for you</h4> 

        <%
    try{
                                Class.forName("com.mysql.jdbc.Driver");
                                Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eb","root","");
                                PreparedStatement ps=con.prepareStatement("select qid,qname,DATE_FORMAT(log,'%b %d, %Y') from ques_ans where status=0 order by log DESC");
                                ResultSet rs=ps.executeQuery();
                                while(rs.next()){
                                    String qid=rs.getString(1);
                                    %>
                                    <hr>
                    <a href="replyans.jsp?qid=<%=qid%>" style="font-size:15px;color: black;text-decoration: none;"><H2><% out.println(rs.getString(2)); %></H2>
                    </a>
