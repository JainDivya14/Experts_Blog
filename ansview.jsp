<%-- 
    Document   : ansview
    Created on : May 14, 2020, 5:02:06 PM
    Author     : Lenovo
--%>
...
<BODY style="background:#bdffc4;">
    ...
    try{
                                ...
                                while(rs.next()){
                                    ...
                                    q=rs.getString(2);
                                    %>
  <a href="ansview.jsp?qid" style="font-size:15px;color: black;text-decoration: none;"><H2><% out.println(q); %></H2>
                    </a>
                    ...
                    <table style="border-collapse: seperate; border-spacing: 25px 0px;">
                        <tr>
                            <td style="background:seagreen;color:black;font-weight: bold;padding: 12px 12px 12px 12px;border-radius: 25px;">
                                <a href="support_ans.jsp?support=<%=support%>&qid=<%=qid%>"><% out.println((support==1) ? "Supported" : "Support"); %>
                                </a></td> <!-- support_ans working is actually supporting ques.-->
                            <td style="background:goldenrod;color:white;font-weight: bold;padding: 12px 12px 12px 12px;border-radius: 25px;">
                                <a href="save_ques.jsp?save=<%=save%>&qid=<%=qid%>"><% out.println((save==1) ? "Saved" : "Save"); %>
                                </a></td>
                        </tr>
                    </table>
                                ...
                                <a href="exp_profile_view.jsp?expid=<%=expid%>" style="color:black;">Answered by <% out.println(ansby); %></a>
                                <p style="font-size: 20px;color: black;"><% out.println(a); %></p>
                                <table style="border-collapse: seperate; border-spacing: 25px 0px;">
                                <tr>
                                <td style="background:skyblue;color:black;font-weight: bold;padding: 12px 12px 12px 12px;border-radius: 25px;"><a href="response.jsp?state=<%=state%>&trigger=<%="like"%>&aid=<%=ansid%>&qid=<%=qid%>"><% out.println((ansres.equals("like")) ? "Upvoted" : "Upvote"); %></a></td>
                                <td style="background:black;color:white;font-weight: bold;padding: 12px 12px 12px 12px;border-radius: 25px;"><a href="response.jsp?state=<%=state%>&trigger=<%="dislike"%>&aid=<%=ansid%>&qid=<%=qid%>"><% out.println((ansres.equals("dislike")) ? "Downvoted" : "Downvote"); %></a></td>
                                </tr>
                                </table>
